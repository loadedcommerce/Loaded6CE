...
$order_total_tmp = $order_total - $exclude_product_price;
  ...
  if ($this->calculate_tax != 'None') { //Fred - changed from 'none' to 'None'!
    ...
    $tod_amount = $this->calculate_tax_deduction($order->info['total'], $this->deduction, $this->calculate_tax);
  }
...
global $customer_id, $currencies, $language;
  ...
  $image_submit2 = tep_template_image_submit('button_redeem.gif', IMAGE_REDEEM_VOUCHER, 'id= "id_button_redeem" onClick="submitFunction()"');
...
$tmp_order_total = $order->info['total'];
...
global $order, $cart;
  ...
  for ($i=0; $i<sizeof($products); $i++) {
    ...
    if (preg_match('/^GIFT/', addslashes($gv_result['products_model']))) {
      $products_price = $gv_result['products_price'];
        $str_exclude_product_price = tep_db_query("select specials_new_products_price from " . TABLE_SPECIALS . " where products_id = '" . $t_prid . "' and status = '1'");
      ...
      if ($this->include_tax == false) {
        $gv_amount = $products_price * $qty;
      } else {
        $gv_amount = ($products_price + tep_calculate_tax($products_price,$products_tax)) * $qty;
      }
