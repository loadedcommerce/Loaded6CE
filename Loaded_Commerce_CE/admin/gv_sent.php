...
<?php
  ...
  while ($gv_list = tep_db_fetch_array($gv_query)) {
    ...
    if ( (isset($gInfo) && is_object($gInfo)) && ($uID == $gv_list['unique_id']) ) {
...
?>
                ...
                <td class="dataTableContent" align="right"><?php if ( (is_object($gInfo)) && ($gv_list['coupon_id'] == $gInfo->coupon_id) ) { echo tep_image(DIR_WS_IMAGES . 'arrow_right_blue.png'); } else { echo '<a href="' . tep_href_link(FILENAME_GV_SENT, 'page=' . $_GET['page'] . '&gid=' . $gv_list['coupon_id']) . '">' . tep_image(DIR_WS_IMAGES . 'information.png', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              </tr>
<?php
  }
...
<?php
  ...
  $contents[] = array('text' => TEXT_INFO_SENDERS_ID . ' ' . $coupon_info['customer_id_sent']);
  $contents[] = array('text' => TEXT_INFO_AMOUNT_SENT . ' ' . $currencies->format($coupon_info['coupon_amount']));
  $contents[] = array('text' => TEXT_INFO_DATE_SENT . ' ' . tep_date_short($coupon_info['date_sent']));
  $contents[] = array('text' => TEXT_INFO_VOUCHER_CODE . ' ' . $coupon_info['coupon_code']);
  $contents[] = array('text' => TEXT_INFO_EMAIL_ADDRESS . ' ' . $coupon_info['emailed_to']);
  if ($redeemed=='Yes') {
    $redeem = tep_db_fetch_array($redeem_query);
    $contents[] = array('text' => '<br>' . TEXT_INFO_ORDER_ID . ' ' . $redeem['order_id']);
    $contents[] = array('text' => TEXT_INFO_DATE_REDEEMED . ' ' . tep_date_short($redeem['redeem_date']));
