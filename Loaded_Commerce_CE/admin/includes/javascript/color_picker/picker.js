...
function TCPopup(field, palette) {
  ...
  o_colWindow = window.open('includes/javascript/color_picker/picker.php', null, "help=no,status=no,scrollbars=no,resizable=no" + move + ",width=" + w + ",height=" + h + ",dependent=yes", true);
  o_colWindow.opener = window;
  o_colWindow.focus();
}

function TCBuildCell (R, G, B, w, h) {
  return '<td width="' + w + '" height="' + h + '" border="0" bgcolor="#' + this.dec2hex((R << 16) + (G << 8) + B) + '"><a href="javascript:P.S(\'' + this.dec2hex((R << 16) + (G << 8) + B) + '\')" onmouseover="P.P(\'' + this.dec2hex((R << 16) + (G << 8) + B) + '\')"><img src="img/pixel.gif" width="' + w + '" height="' + h + '" border="0"></a></td>';
}
