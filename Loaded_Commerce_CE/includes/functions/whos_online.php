...
*/
  ...
  function tep_update_whos_online() {
    ...
    $wo_last_page_url = htmlentities(getenv('REQUEST_URI'), ENT_QUOTES);
...
// remove entries that have expired
    ...
    if ($stored_customer['count'] > 0) {
      ...
      tep_db_query("insert into " . TABLE_WHOS_ONLINE . " (customer_id, full_name, session_id, ip_address, time_entry, time_last_click, last_page_url, http_referer) values ('" . (int)$wo_customer_id . "', '" . tep_db_input($wo_full_name) . "', '" . tep_db_input($wo_session_id) . "', '" . tep_db_input($wo_ip_address) . "', '" . tep_db_input($current_time) . "', '" . tep_db_input($current_time) . "', '" . tep_db_input($wo_last_page_url) . "', '" . tep_db_input($http_referer) . "')");
    }
  }
?>
