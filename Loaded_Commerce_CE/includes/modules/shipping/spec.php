...
// class constructor
    function spec() {
      ...
      if (defined('MODULE_SHIPPING_SPEC_SORT_ORDER')) {
        $this->sort_order = (int)MODULE_SHIPPING_SPEC_SORT_ORDER;
      } else {
        $this->sort_order = '';
      }

      $this->icon = '';
      if (defined('MODULE_SHIPPING_SPEC_TAX_CLASS')) {
        $this->tax_class = (int)MODULE_SHIPPING_SPEC_TAX_CLASS;
      } else {
        $this->tax_class = 0;
      }

      if (defined('MODULE_SHIPPING_SPEC_STATUS')) {
...
// END OF ADDITIONAL INSURANCE CHARGES
      ...
      if ($shipping_weight <= $zones_table[$i]) {
         ...
         //  $shipping_method = 'Urgent or higher value';
            $shipping_method = MODULE_SHIPPING_SPEC_SHIPPING_METHOD;
...
if ($order->info['total'] > 2400) {
 $error = true;
         ...
         $shipping_cost = ($shipping_cost * $shipping_num_boxes) + $ic + constant('MODULE_SHIPPING_SPEC_HANDLING_' . $dest_zone);
