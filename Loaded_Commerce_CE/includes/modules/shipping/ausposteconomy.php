...
// class constructor
    function ausposteconomy() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_SHIPPING_AUSPOST_ECONOMY_ZONE > 0) && is_object($order) ) {
        $check_flag = false;
        $check_query = tep_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_SHIPPING_AUSPOST_ECONOMY_ZONE . "' and ( zone_country_id = 0 or zone_country_id = '" . $order->delivery['country']['id'] . "' ) order by zone_id");
...
// class methods
    function quote($method = '') {
      ...
      $insurance_table = preg_split("/[:,]/" , MODULE_SHIPPING_AUSPOST_ECONOMY_INSURANCE);
