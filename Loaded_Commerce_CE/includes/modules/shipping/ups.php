...
*/
  ...
  class ups {
    ...
    function ups() {
      ...
      $this->sort_order =  (defined('MODULE_SHIPPING_UPS_SORT_ORDER')) ? (int)MODULE_SHIPPING_UPS_SORT_ORDER : 0;
      $this->icon = DIR_WS_ICONS . 'shipping_ups.gif';
      $this->tax_class = (defined('MODULE_SHIPPING_UPS_TAX_CLASS')) ? (int)MODULE_SHIPPING_UPS_TAX_CLASS : 0;
      ...
      if ( ($this->enabled == true) && ((int)MODULE_SHIPPING_UPS_ZONE > 0) && is_object($order) ) {
        $check_flag = false;
        $check_query = tep_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_SHIPPING_UPS_ZONE . "' and ( zone_country_id = 0 or zone_country_id = '" . $order->delivery['country']['id'] . "' ) order by zone_id");
    ...
    function quote($method = '') {
      ...
      if (defined('INSTALLED_VERSION_TYPE') && stristr(INSTALLED_VERSION_TYPE, 'B2B')) {
         ...
         $my_zip = SHIPPING_ORIGIN_ZIP;
        if (isset($order->delivery['country_id']) && isset($order->delivery['zone_id'])) {
            $sql_query = tep_db_query("select warehouse_zone_id from " . TABLE_ZONES_TO_WAREHOUSE_ZONES . " where (zone_country_id = " . $order->delivery['country_id'] . " and zone_id = " . $order->delivery['zone_id'] . ") or (zone_country_id = " . $order->delivery['country_id'] . " and zone_id = 0)");
          ...
          if ($warehouse_zone_id != null) {
               $sql_query = tep_db_query("select warehouse_zone_zip_code from " . TABLE_WAREHOUSE_ZONES . " where warehouse_zone_id = " . $warehouse_zone_id);
               ...
               if ($warehouse_zone_zip_code != null) {
                 $my_zip = $warehouse_zone_zip_code;
                 $country_name = tep_get_countries($order->delivery['country_id'], true);
               }
            }
         }
      }
    ...
    function install() {
      ...
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('UPS Shipping Methods', 'MODULE_SHIPPING_UPS_TYPES', 'GND', 'Select the UPS services to be offered.<br><br>Nxt AM, Nxt AM Ltr, Nxt, Nxt Ltr, Nxt PR, Nxt Save, Nxt Save Ltr, 2nd AM, 2nd AM Ltr, 2nd, 2nd Ltr, 3 Day Select, Ground, Canada, World Xp, World Xp Ltr, World Xp Plus, World Xp Plus Ltr, World Expedite.', '6', '13', 'tep_cfg_select_multioption(array(\'1DM\',\'1DML\', \'1DA\', \'1DAL\', \'1DAPI\', \'1DP\', \'1DPL\', \'2DM\', \'2DML\', \'2DA\', \'2DAL\', \'3DS\',\'GND\', \'STD\', \'XPR\', \'XPRL\', \'XDM\', \'XDML\', \'XPD\'), ', now() )");
    }
