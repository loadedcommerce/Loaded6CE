...
// class constructor
    function table() {
      ...
      if (defined('MODULE_SHIPPING_TABLE_SORT_ORDER')) {
        $this->sort_order = (int)MODULE_SHIPPING_TABLE_SORT_ORDER;
      } else {
        $this->sort_order = '';
      }
      $this->icon = '';
      if (defined('MODULE_SHIPPING_TABLE_TAX_CLASS')) {
        $this->tax_class = (int)MODULE_SHIPPING_TABLE_TAX_CLASS;
      } else {
        $this->tax_class = 0;
      }
      ...
      if ( ($this->enabled == true) && ((int)MODULE_SHIPPING_TABLE_ZONE > 0) && is_object($order) ) {
        $check_flag = false;
        $check_query = tep_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_SHIPPING_TABLE_ZONE . "' and ( zone_country_id = 0 or zone_country_id = '" . $order->delivery['country']['id'] . "' ) order by zone_id");
    ...
    function quote($method = '') {
      ...
      for ($i=0, $n=$size; $i<$n; $i+=2) {
        if ($order_total <= $table_cost[$i]) {
          $shipping_cost = $table_cost[$i+1];
          break;
        }
      }

      if (MODULE_SHIPPING_TABLE_MODE == 'weight') {
        $shipping_cost = $shipping_cost * $shipping_num_boxes;
      }

      $shippingcost = (float)$shipping_cost + (float)MODULE_SHIPPING_TABLE_HANDLING;
