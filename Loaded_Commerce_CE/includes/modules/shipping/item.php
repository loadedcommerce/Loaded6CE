...
// class constructor
    function item() {
      ...
      if (defined('MODULE_SHIPPING_ITEM_SORT_ORDER')) {
        $this->sort_order = (int)MODULE_SHIPPING_ITEM_SORT_ORDER;
      } else {
        $this->sort_order = '';
      }
      ...
      $this->icon = '';
      if (defined('MODULE_SHIPPING_ITEM_TAX_CLASS')) {
        $this->tax_class = (int)MODULE_SHIPPING_ITEM_TAX_CLASS;
      } else {
        $this->tax_class = 0;
      }
      ...
      if ( ($this->enabled == true) && ((int)MODULE_SHIPPING_ITEM_ZONE > 0) && is_object($order) ) {
        $check_flag = false;
        $check_query = tep_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_SHIPPING_ITEM_ZONE . "' and ( zone_country_id = 0 or zone_country_id = '" . $order->delivery['country']['id'] . "' ) order by zone_id");
...
// class methods
    function quote($method = '') {
      ...
      $shipping_cost = (MODULE_SHIPPING_ITEM_COST * $total_count) + MODULE_SHIPPING_ITEM_HANDLING;
