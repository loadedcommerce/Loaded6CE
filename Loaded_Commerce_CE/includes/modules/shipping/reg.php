...
// class constructor
    function reg() {
      ...
      if (defined('MODULE_SHIPPING_REG_SORT_ORDER')) {
        $this->sort_order = (int)MODULE_SHIPPING_REG_SORT_ORDER;
      } else {
        $this->sort_order = '';
      }

      $this->icon = '';
      if (defined('MODULE_SHIPPING_REG_TAX_CLASS')) {
        $this->tax_class = (int)MODULE_SHIPPING_REG_TAX_CLASS;
      } else {
        $this->tax_class = 0;
      }

      if (defined('MODULE_SHIPPING_REG_STATUS')) {
    ...
    function quote($method = '') {
      ...
      if ($shipping_weight <= $zones_table[$i]) {
           ...
           // $shipping_method = 'Light, lower value deliveries';
            $shipping_method = MODULE_SHIPPING_REG_SHIPPING_METHOD;
...
if ($order->info['total'] > 28) {
 $error = true;
         ...
         $shipping_cost = ($shipping_cost * $shipping_num_boxes) + constant('MODULE_SHIPPING_REG_HANDLING_' . $dest_zone);
