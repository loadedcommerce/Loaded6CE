...
// class constructor
    function canadapost() {
      ...
      $this->comments = '';
      if (defined('MODULE_SHIPPING_CANADAPOST_SORT_ORDER')) {
        $this->sort_order = (int)MODULE_SHIPPING_CANADAPOST_SORT_ORDER;
      } else {
        $this->sort_order = '';
      }

      if (defined('MODULE_SHIPPING_CANADAPOST_TAX_CLASS')) {
        $this->tax_class = (int)MODULE_SHIPPING_CANADAPOST_TAX_CLASS;
      } else {
        $this->tax_class = 0;
      }
      ...
      if (defined('MODULE_SHIPPING_CANADAPOST_SERVERIP')) {
        $this->server = MODULE_SHIPPING_CANADAPOST_SERVERIP;
      } else {
        $this->server = '';
      }
      if (defined('MODULE_SHIPPING_CANADAPOST_SERVERPOST')) {
        $this->port = (int)MODULE_SHIPPING_CANADAPOST_SERVERPOST;
      } else {
        $this->port = 0;
      }

      if (defined('MODULE_SHIPPING_CANADAPOST_LANGUAGE')) {
        $this->language = MODULE_SHIPPING_CANADAPOST_LANGUAGE;
      } else {
        $this->language = '';
      }

      if (defined('MODULE_SHIPPING_CANADAPOST_CPCID')) {
        $this->CPCID = MODULE_SHIPPING_CANADAPOST_CPCID;
      } else {
        $this->CPCID = 0;
      }

      if (defined('MODULE_SHIPPING_CANADAPOST_TIME')) {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_SHIPPING_CANADAPOST_ZONE > 0) && is_object($order) ) {
        $check_flag = false;
        $check_query = tep_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_SHIPPING_CANADAPOST_ZONE . "' and ( zone_country_id = 0 or zone_country_id = '" . $order->delivery['country']['id'] . "' ) order by zone_id");
...
// class methods
    function quote($method = '') {
      ...
      $this->_canadapostDest((isset($order->delivery['city']) ? $order->delivery['city'] : ''), (isset($order->delivery['state']) ? $order->delivery['state'] : ''), $order->delivery['country']['iso_code_2'], $order->delivery['postcode']);
      ...
      for ($i=0; $i<count($products_array); $i++)
        $this->_addItem ($products_array[$i]['quantity'], $products_array[$i]['final_price'], $products_array[$i]['weight'],
              (isset($this->default_length) ? $this->default_length : 0), (isset($this->default_width) ? $this->default_width : 0), (isset($this->default_height) ? $this->default_height : 0), $products_array[$i]['name']);
    ...
    if ( $method == '' || $method == $type ) {
            $methods[] = array('id' => $type,
                             'title' => $type,
                             'cost' => (CANADAPOST_SHIPPING_HANDLING + $cost));
    }
        }
    ...
    function install() {
      ...
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Enter CanadaPost Server IP', 'MODULE_SHIPPING_CANADAPOST_SERVERIP', '66.110.6.131', 'ip address of canada post server', '6', '11', now())");
      ...
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('shipping handling cost', 'CANADAPOST_SHIPPING_HANDLING', '0', 'What is the cost of handling fee?', '6', '0', now())");
