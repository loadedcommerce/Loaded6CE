    ...
    function before_process() {
      ...
      if (tep_db_num_rows($check_query)) {
        ...
        if ($check['orders_status'] == MODULE_PAYMENT_WORLDPAY_JUNIOR_PREPARE_ORDER_STATUS_ID) {
          ...
          if (isset($_GET['hash']) && !empty($_GET['hash']) && ($_GET['hash'] == md5($_GET['gsid'] . $customer_id . $order_id . $language . number_format($order->info['total'], 2) . MODULE_PAYMENT_WORLDPAY_JUNIOR_MD5_PASSWORD))) {
            $hash_result = true;
          }
      ...
      if (defined('MVS_STATUS') && MVS_STATUS == 'true') {
        if ((defined('MVS_VENDOR_EMAIL_WHEN') && MVS_VENDOR_EMAIL_WHEN == 'Catalog') || 
          ...
          if (isset($status)) {
            $order_sent_query = tep_db_query("select vendor_order_sent, vendors_id from " . TABLE_ORDERS_SHIPPING . " where orders_id = '" . $order_id . "'");
