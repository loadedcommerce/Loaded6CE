...
// class constructor
    function moneyorder() {
      ...
      if (defined('MODULE_PAYMENT_MONEYORDER_TEXT_DESCRIPTION')) {
        $this->description = MODULE_PAYMENT_MONEYORDER_TEXT_DESCRIPTION;
      } else {
        $this->description = '';
      }
      if (defined('MODULE_PAYMENT_MONEYORDER_SORT_ORDER')) {
        $this->sort_order = (int)MODULE_PAYMENT_MONEYORDER_SORT_ORDER;
      } else {
        $this->sort_order = '';
      }
      ...
      if (defined('MODULE_PAYMENT_MONEYORDER_TEXT_EMAIL_FOOTER')) {
...
// class methods
    function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_MONEYORDER_ZONE > 0) ) {
        $check_flag = false;
        $check_query = tep_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_MONEYORDER_ZONE . "' and ( zone_country_id = 0 or zone_country_id = '" . $order->billing['country']['id'] . "' ) order by zone_id");
    ...
    function install() {
      ...
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Money Order  Make Payable to:', 'MODULE_PAYMENT_MONEYORDER_PAYTO', '" . STORE_NAME . "', 'Who should payments be made payable to?', '6', '1', now());");
