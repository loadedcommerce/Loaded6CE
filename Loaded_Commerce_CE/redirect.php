  ...
  switch ($_GET['action']) {
    ...
    case 'url':
      if (isset($_GET['goto']) && tep_not_null($_GET['goto'])) {
        // the product page does an urlencode, so it must be decode for a match to occur
        $check_query = tep_db_query("select products_url from " . TABLE_PRODUCTS_DESCRIPTION . " where products_url = '" . tep_db_input(urldecode($_GET['goto'])) . "' limit 1");
        if (tep_db_num_rows($check_query) > 0) {
          $check = tep_db_fetch_array($check_query);
          tep_redirect('http://' . $check['products_url']);
        }
      }
      break;
...
// no url exists for the selected language, lets use the default language then
    ...
    case 'article':
      if (isset($_GET['goto']) && tep_not_null($_GET['goto'])) {
        $check_query = tep_db_query("select articles_url from " . TABLE_ARTICLES_DESCRIPTION . " where articles_url = '" . tep_db_input(urldecode($_GET['goto'])) . "' limit 1");
        if (tep_db_num_rows($check_query) > 0) {
          $check = tep_db_fetch_array($check_query);
          tep_redirect('http://' . $check['articles_url']);
        }
      }
      break;
  }
